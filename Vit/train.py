...
def train():
    loss_epoch = 0
    for step, ((x_i, x_j, _), _) in enumerate(data_loader):
        optimizer.zero_grad()
        x_i = x_i.to('cuda')
        x_j = x_j.to('cuda')
        ...
        loss = loss_instance + loss_cluster + 10*loss_hcr#CC+BCE
        loss.backward()
        optimizer.step()
        if step % 50 == 0:
            print(
                f"Step [{step}/{len(data_loader)}]\t loss_instance: {loss_instance.item()}\t loss_cluster: {loss_cluster.item()} \t loss_hcr: {10*loss_hcr.item()}")
